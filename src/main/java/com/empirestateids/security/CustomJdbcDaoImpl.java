...
public class CustomJdbcDaoImpl extends JdbcDaoSupport implements UserDetailsService {
	public static final String DEF_USERS_BY_USERNAME_QUERY =
            "select u.username, u.password, u.enable, " +
            "	u.account_non_expired, u.account_non_locked, u.credentials_non_expired, u.password_reset, " +
            "	u.ipowerlift_id, p.first_name, p.last_name, e.email, " +
            "	gm.group_id " +
            "from users u, person p, email e, group_member gm " +
            "where u.ipowerlift_id=p.ipowerlift_id and " +
            "	p.contact_id=e.contact_id and " +
            "	u.username=gm.username and " + 
            "	upper(u.username) = upper(?)";
    ...
    public static final String DEF_GROUP_AUTHORITIES_BY_USERNAME_QUERY =
            ...
            "from `groups` g, group_member gm, group_authority ga " +
            "where gm.username = ? " +
    ...
    protected List<UserDetails> loadUsersByUsername(String username) {
        return getJdbcTemplate().query(usersByUsernameQuery, new String[] {username}, new RowMapper<UserDetails>() {
            public UserDetails mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                boolean enabled = rs.getBoolean("enable");
                ...
                int ipowerliftId  = rs.getInt("ipowerlift_id");
                ...
                return new UserInfo(username, password, enabled, accountNonExpired, credentialsNonExpired, accountNonLocked, passwordReset, ipowerliftId, firstName, lastName, email, AuthorityUtils.NO_AUTHORITIES, groupId);
            }
    ...
    protected UserDetails createUserDetails(String username, UserDetails userFromUserQuery,
    	...
    	UserInfo userInfo = (UserInfo) userFromUserQuery;
        String returnUsername = userInfo.getUsername();

        if (!usernameBasedPrimaryKey) {
            returnUsername = username;
        }

        return new UserInfo(returnUsername, userInfo.getPassword(), userInfo.isEnabled(),
        		userInfo.isAccountNonExpired(), userInfo.isCredentialsNonExpired(), userInfo.isAccountNonLocked(), userInfo.isPasswordReset(),
        		userInfo.getIpowerliftId(), userInfo.getFirstName(), userInfo.getLastName(), userInfo.getEmail(),
        		combinedAuthorities, userInfo.getGroupId());
    }
