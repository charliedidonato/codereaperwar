...
public class ExceptionHandlingController extends AtlasController {
  ...
  public ModelAndView handleError(HttpServletRequest req, GenericException ex) {
    logger.error("Request: " + req.getRequestURL() + " raised " + ex);
    logger.error("Message:"+ex.getMessage()+
    		" Trace:" + UtilityMethods.getStackTrace(ex));
    ModelAndView mav = new ModelAndView();
    mav.addObject("exception", ex);
    mav.addObject("message", ex.getMessage());
    mav.addObject("userfriendlymessage", ex.getCustomMsg());
    mav.addObject("url", req.getRequestURL());
    mav.setViewName("/error/genericExceptionPage.jsp");
    return mav;
  }
}